...
Browser → http://34.227.190.40
         ...
         Nginx :80  (Docker container)
          ...
          └── /api/*     → http://backend:8080
               │
         Spring Boot :8080  (Docker container)
               │
         MariaDB 10.11 :3306  (Docker container)
```

All three services run as Docker containers on a single EC2 instance, connected via a Docker bridge network (`gotokart-net`). Nginx is the only container exposed to the internet (port 80).
