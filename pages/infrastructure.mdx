...
| **AMI** | Amazon Linux 2023 — kernel 6.1 (`al2023-ami-2023.10.20260330.0`) |
...
| **VPC** | `vpc-0f3b682faa9b16f81` |
| **Subnet** | `subnet-02ac3f55e7d3b8c66` |
| **Launch time** | 2026-04-07 22:28 IST |

## Architecture

```
Browser  ──HTTP──▶  EC2 :80 (Nginx container)
                        │
                   /api/* ──proxy_pass──▶  backend:8080 (Spring Boot container)
                   /       ──static files         │
                                                   └──JPA──▶  mariadb:3306 (MariaDB container)
```

All three services run as Docker containers on a single EC2 instance, connected via a Docker bridge network (`gotokart-net`). Nginx is the only container with a public port (80).
